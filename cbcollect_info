...
def make_product_task(guts, initargs_path, options):
    ...
    if index_port:
        password = read_guts(guts, "memcached_pass")
        index_tasks = [make_index_task("Index definitions are: ", "getIndexStatus", password, index_port),
                       make_index_task("Indexer settings are: ", "settings", password, index_port),
                       make_index_task("Index storage stats are: ", "stats/storage", password, index_port),
                       make_index_task("MOI allocator stats are: ", "stats/storage/mm", password, index_port),
                       make_index_task("Indexer Go routine dump: ", "debug/pprof/goroutine?debug=1",
                                       password, index_port, logfile="indexer_pprof.log")]
    ...
    if proj_port:
        ...
        projector_tasks = [make_curl_task(name="Projector Go routine dump ",
                                          user="@", password=read_guts(guts, "memcached_pass"),
