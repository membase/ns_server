...
insert_500_both_ways_test() ->
  ...
  _LeafHashB = lists:foldl(fun({_,Hash}, Sum) ->
      (Hash + Sum) rem (2 bsl 31)
    end, 0, leaves(Pid2)),
...
open_and_insert_n(X) ->
  ...
  ?assertEqual(true, lists:all(fun(N) ->
      ...
      if
        Result -> Result;
        true ->
          error_logger:info_msg("could not get ~p was ~p~n",
                                [N, find(lists:concat(["key", N]), Pid)]),
          Result
      end
    end, lists:seq(1, X))),
  close(Pid).
