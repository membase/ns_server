...
%% @doc Will make synchronous call to nodes and aggregate. Aggregate function must be a
%% commutative function of arity 2.
aggregate_call(Nodes, Module, Request, AFun, Timeout) ->
    {Replies, BadNodes} = gen_server:multi_call(Nodes, Module, Request, Timeout),
    [First | Rest] = [Reply || {_Node, Reply} <- Replies],
    {lists:foldl(AFun, First, Rest), BadNodes}.
